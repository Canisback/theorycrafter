	...
	function createSpells(){
		...
		for(var i in CHAMPION_SPELLS){
			if(CHAMPION_SPELLS[i]["key"]!="Passive_"+CHAMPION_ID){
				$("#champion_spells").append('<hr><table id="'+CHAMPION_SPELLS[i]["key"]+'"><tbody><tr><td style=\'background : url("'+Canis.LoL.TheoryCrafter.getQuery()+'application/ddragon/'+Canis.LoL.TheoryCrafter.getDDragon()+'/img/sprite/'+CHAMPION_SPELLS[i]['sprite']+'") no-repeat scroll -'+CHAMPION_SPELLS[i]['x']+'px -'+CHAMPION_SPELLS[i]['y']+'px transparent;min-height:48px;min-width:48px;max-height:48px;max-width:48px;display:inline-block;\'></td><td style="padding-left:5px;">'+stripslashes(CHAMPION_SPELLS[i]["name"])+'<br />'+stripslashes(CHAMPION_SPELLS[i]["description"])
					+'<br />Coût : '+stripslashes(CHAMPION_SPELLS[i]['resource'])+'<br />Récupération : <span class="cooldown"></span> secondes<br /><input value="0" type="range" class="spell_level_range" min="0" max="'+CHAMPION_SPELLS[i]["maxrank"]+'" autocomplete="off" style="max-width:200px;display:inline" data-key="'+CHAMPION_SPELLS[i]["key"]+'" data-type="spell"><span class="spell_level"></span></td></tr></tbody></table>');
